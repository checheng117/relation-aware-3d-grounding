#!/usr/bin/env python3
"""Clean attribute-only baseline for 3D grounding."""
...
from rag3d.relation_reasoner.attribute_scorer import AttributeOnlyModel
from rag3d.training.runner import TrainingConfig, build_loaders, forward_attribute, run_training_loop
...
def run_attribute_baseline(config_path: Path, output_dir: Path, run_train: bool = True, run_eval: bool = True):
    """Run the attribute-only baseline training and evaluation."""
    setup_logging()
    ...
    model_key = str(config.get("model", "attribute_only"))
    ...
    # Training
    if run_train:
        print("Starting attribute-only baseline training...")
        ...
        if not train_path.is_file():
            ...
            # Run training
            run_training_loop(
                model, train_loader, val_loader, training_config, forward_fn, parser=None, model_name="attribute_baseline"
            )

            # Save final model
            final_checkpoint_path = training_config.checkpoint_dir / "attribute_baseline_final.pt"
            torch.save({
    ...
    # Evaluation
    if run_eval:
        print("Starting attribute-only baseline evaluation...")

        # Load trained model if available, otherwise initialize randomly
        if run_train:
            checkpoint_path = training_config.checkpoint_dir / "attribute_baseline_final.pt"
        ...
        if dataset:
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(eval_loader):
                    ...
                    _, top_k_indices = torch.topk(probs, k=min(5, probs.size(-1)), dim=-1)

                    # Store predictions and targets
                    for i in range(len(batch.samples)):
                        sample = batch.samples[i]

                        # Get the top predictions for this sample
                        ...
                        top5_indices = torch.topk(masked_probs, k=min(5, len(sample_mask)), largest=True).indices
            ...
            # Export results
            eval_output_dir = output_dir / "attribute_baseline"
            eval_output_dir.mkdir(parents=True, exist_ok=True)

            export_results_to_json(results, eval_output_dir / 'attribute_baseline_results.json')
            export_results_to_markdown(results, eval_output_dir / 'attribute_baseline_results.md')
            ...
            print("Skipping evaluation due to missing dataset")


def main():
    parser = argparse.ArgumentParser(description='Attribute-only baseline for 3D grounding')
    parser.add_argument('--config', type=Path,
                       default=ROOT / 'configs/train/attribute_baseline.yaml',
    ...
    if args.output_dir:
        ...
        output_dir = ROOT / "outputs" / f"{timestamp}_attribute_baseline"
...
def create_basic_config(config_path: Path):
    """Create a basic configuration file for the attribute baseline."""
    basic_config = {
        ...
        "seed": 42,
        "device": "cpu",
        "checkpoint_dir": "outputs/checkpoints/attribute_baseline",
        "metrics_file": "outputs/metrics/attribute_baseline_metrics.jsonl",
        "eval_split": "val",
