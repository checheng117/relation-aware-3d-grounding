#!/usr/bin/env python3
"""Raw-text relation baseline for 3D grounding."""
...
from rag3d.relation_reasoner.model import RawTextRelationModel
from rag3d.training.runner import TrainingConfig, build_loaders, forward_raw_text_relation, run_training_loop
...
def run_raw_text_relation_baseline(config_path: Path, output_dir: Path, run_train: bool = True, run_eval: bool = True):
    """Run the raw-text relation baseline training and evaluation."""
    setup_logging()
    ...
    model_key = str(config.get("model", "raw_text_relation"))
    ...
    # Training
    if run_train:
        print("Starting raw-text relation baseline training...")
        ...
        if not train_path.is_file():
            ...
            # Run training
            run_training_loop(
                model, train_loader, val_loader, training_config, forward_fn, parser=None, model_name="raw_text_relation_baseline"
            )

            # Save final model
            final_checkpoint_path = training_config.checkpoint_dir / "raw_text_relation_baseline_final.pt"
            torch.save({
    ...
    # Evaluation
    if run_eval:
        print("Starting raw-text relation baseline evaluation...")

        # Load trained model if available, otherwise initialize randomly
        if run_train:
            checkpoint_path = training_config.checkpoint_dir / "raw_text_relation_baseline_final.pt"
        ...
        if dataset and len(dataset) > 0:
            ...
            # Export results
            eval_output_dir = output_dir / "raw_text_relation_baseline"
            eval_output_dir.mkdir(parents=True, exist_ok=True)

            export_results_to_json(results, eval_output_dir / 'raw_text_relation_baseline_results.json')
            export_results_to_markdown(results, eval_output_dir / 'raw_text_relation_baseline_results.md')
...
def main():
    parser = argparse.ArgumentParser(description='Raw-text relation baseline for 3D grounding')
    parser.add_argument('--config', type=Path,
                       default=ROOT / 'configs/train/raw_text_relation_baseline.yaml',
    ...
    if args.output_dir:
        ...
        output_dir = ROOT / "outputs" / f"{timestamp}_raw_text_relation_baseline"
    ...
    # Run the baseline
    run_raw_text_relation_baseline(
        args.config,
        output_dir,
...
def create_basic_config(config_path: Path):
    """Create a basic configuration file for the raw-text relation baseline."""
    basic_config = {
        ...
        "seed": 42,
        "device": "cpu",
        "checkpoint_dir": "outputs/checkpoints/raw_text_relation_baseline",
        "metrics_file": "outputs/metrics/raw_text_relation_baseline_metrics.jsonl",
        "eval_split": "val",
