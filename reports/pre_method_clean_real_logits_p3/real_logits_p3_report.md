...
This entry checks that real exported base logits can drive the Base / Dense-no-cal / Dense-calibrated comparison path. The relation signal is still the existing oracle-anchor geometry proxy, so these numbers are not method evidence.
